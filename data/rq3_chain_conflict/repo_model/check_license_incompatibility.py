...
def check_license_compatibility(license_a, terms_a, license_b, terms_b, license_a_default_flags, license_b_default_flags):
    ...
    if conflict_terms:
        ...
        conflict_pair_dict[key] = []  # 兼容的情况存入空列表
        conflict_pair_count[key] = 0  # 兼容时冲突次数为 0
...
def process_chains():
    ...
    compatibility_stats = {
        ...
        '3-nodes': {
            'compatible': 0,
            'incompatible': 0,
            'dataset_model_compatible_but_repo_incompatible': 0,
            'model_repo_compatible_but_dataset_incompatible': 0,
        }
    }
    ...
    # 根据 sorted_keys 的顺序重新组织 conflict_pair_dict
