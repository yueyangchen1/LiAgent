...
basemodel_model_pair_false_list = []
conflict_positions = {}
...
def process_chains():
    ...
    for chain in chains:
        ...
        for model_id in chain:
            ...
            if mapped_license == "":
                licenses.append("false")
                original_licenses.append("false")
                continue
        ...
        for i in range(len(chain) - 1, 0, -1):
            ...
            if licenses[i] != "false" and licenses[i - 1] != "false":
                try:
                    ...
                    print(f"[跳过链] 处理 license 转换时报错: {e}")
                    skip_chain = True
                    break
            else:
                if basemodel_model_pair not in basemodel_model_pair_false_list:
                    basemodel_model_pair_false_list.append(basemodel_model_pair)
    ...
    print(f"has {len(basemodel_model_pair_false_list)}pair cant check")
