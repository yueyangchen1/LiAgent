...
mismatch_dict_dataset_file = '../merged_mismatch_dict_dataset_new.json'
...
conflict_pair_dict = {}  # 记录具体哪些 term 冲突，键为 (license_a, license_b)
conflict_pair_count = {}  # 记录 (license_a, license_b) 冲突的次数
conflict_term_count = {}
...
def check_term_compatibility(terms_a, terms_b):
    conflict_terms = []  # 记录冲突的 term
    conflict_terms_index = []
    for i in range(len(terms_a)):  # i 从 0 开始
        if terms_a[i] == 1:  # 1 兼容所有值
            continue
...
def process_chains():
    ...
    for chain in chains:
        ...
        repo_license = get_model_license(repo_id, models, mismatch_dict_model)
        model_license = get_dataset_license(model_id, datasets, mismatch_dict_dataset)
    ...
    for pair_str, term_list in conflict_pair_dict.items():
        ...
        if pair_str not in conflict_pair_count:
            print(f"pair{pair_str}not found")
            continue
